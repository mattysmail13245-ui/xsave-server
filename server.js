...
app.post('/profile', async (req, res) => {
  ...
  try {
    ...
    const uploader = items.length ? (() => { try { return JSON.parse(raw.split('\n').find(l=>l.startsWith('{'))).uploader||''; } catch { return ''; } })() : '';
...
app.get('/preview', (req, res) => {
  ...
  proc.stdout.on('data', chunk => {
    if (killed) return;
    bytes += chunk.length;
    if (bytes > MAX) { killed = true; proc.kill(); if (!res.writableEnded) res.end(); return; }
    res.write(chunk);
  });
...
app.listen(PORT, () => console.log(`XLoad v4 :${PORT} | cookies:${fs.existsSync(COOKIES_PATH)}`));
...
:root{--bg:#000;--c1:#111;--c2:#181818;--bd:#1e1e1e;--tx:#fff;--sub:#555;--g:#00e87a;--r:#ff3b3b}
...
.sp-dev{font-size:10px;font-weight:600;color:#1e1e1e;letter-spacing:3.5px;text-transform:uppercase;opacity:0;animation:fi .5s .4s forwards}
.sp-name{font-size:56px;font-weight:800;letter-spacing:-4px;line-height:1;opacity:0;animation:su .8s .7s cubic-bezier(.34,1.56,.64,1) forwards}
.sp-bar{width:36px;height:1.5px;background:#111;border-radius:2px;margin-top:6px;overflow:hidden;opacity:0;animation:fi .3s 1.1s forwards}
...
.brand .dev{font-size:10px;font-weight:600;color:#1e1e1e;letter-spacing:2.5px;text-transform:uppercase}
.brand .nm{font-size:22px;font-weight:800;letter-spacing:-1.2px;line-height:1.1}
.tr{display:flex;align-items:center;gap:10px}
.cdot{width:8px;height:8px;border-radius:50%;background:#1e1e1e;transition:.3s;flex-shrink:0}
.cdot.on{background:var(--g);box-shadow:0 0 8px var(--g)}
.ibtn{width:36px;height:36px;background:var(--c1);border:1px solid var(--bd);border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:15px;transition:background .15s}
.ibtn:active{background:var(--c2)}

/* TABS */
.tabs{display:flex;gap:5px;padding:4px 14px 8px;overflow-x:auto;flex-shrink:0;scrollbar-width:none}
.tabs::-webkit-scrollbar{display:none}
.tab{flex:1;min-width:58px;padding:9px 5px 8px;border-radius:13px;background:var(--c1);border:1px solid var(--bd);font-family:inherit;font-size:11px;font-weight:600;color:var(--sub);cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:3px;transition:all .18s;white-space:nowrap}
...
/* INPUT */
.icard{background:var(--c1);border:1px solid var(--bd);border-radius:18px;padding:14px;display:flex;flex-direction:column;gap:9px}
.irow{display:flex;gap:8px;align-items:center}
.uinp{flex:1;min-width:0;background:var(--c2);border:1.5px solid var(--bd);border-radius:12px;color:#fff;font-family:inherit;font-size:15px;padding:12px 13px;outline:none;-webkit-appearance:none;transition:border-color .15s}
...
.pbtn{background:var(--c2);border:1px solid var(--bd);border-radius:10px;color:var(--sub);font-family:inherit;font-size:14px;font-weight:500;padding:11px;cursor:pointer;transition:background .15s,color .15s}
.pbtn:active{background:#222;color:#fff}

/* STATUS */
.stat{display:none;align-items:center;gap:8px;background:var(--c1);border:1px solid var(--bd);border-radius:100px;padding:9px 14px;font-size:13px;font-weight:500;color:var(--sub)}
...
/* RESULT */
.res{display:none;background:var(--c1);border:1px solid var(--bd);border-radius:18px;overflow:hidden;animation:pop .32s cubic-bezier(.34,1.56,.64,1)}
...
.pvplay span{font-size:52px;filter:drop-shadow(0 2px 12px rgba(0,0,0,.6))}
.pvplay.hide{opacity:0;pointer-events:none}
.pvbadge{position:absolute;bottom:10px;left:10px;background:rgba(0,0,0,.55);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,.1);border-radius:100px;padding:4px 10px;font-size:10px;font-weight:700;letter-spacing:.8px;text-transform:uppercase}
.pvprog{position:absolute;bottom:0;left:0;height:2px;background:var(--g);width:0;transition:width .5s linear}

/* RESULT BODY */
.rbody{padding:14px;display:flex;flex-direction:column;gap:8px}
.rmeta{display:flex;align-items:center;gap:6px}
.rdot{width:6px;height:6px;border-radius:50%;background:var(--g);flex-shrink:0}
.rsub{font-size:12px;color:var(--sub);font-weight:500}

/* DL BTN */
.dl{display:flex;align-items:center;justify-content:space-between;background:var(--c2);border:1px solid var(--bd);border-radius:13px;padding:13px;text-decoration:none;color:inherit;transition:background .15s;gap:10px;margin-bottom:7px}
...
.pi{aspect-ratio:1;border-radius:10px;overflow:hidden;background:var(--c2);position:relative;text-decoration:none;display:block;border:1px solid var(--bd)}
.pi img{width:100%;height:100%;object-fit:cover;display:block}
.piov{position:absolute;inset:0;background:rgba(0,0,0,.5);display:flex;align-items:center;justify-content:center;font-size:20px;opacity:0;transition:opacity .15s}
.pi:active .piov{opacity:1}

/* PROFILE GRID */
.pgrid2{display:grid;grid-template-columns:repeat(3,1fr);gap:2px}
.pitem{aspect-ratio:1;background:var(--c2);position:relative;overflow:hidden;cursor:pointer}
.pitem img{width:100%;height:100%;object-fit:cover;display:block;background:#111}
.pitov{position:absolute;inset:0;background:rgba(0,0,0,.5);display:flex;align-items:center;justify-content:center;font-size:15px;opacity:0;transition:opacity .15s}
.pitem:active .pitov{opacity:1}
.pvid{position:absolute;bottom:3px;right:4px;background:rgba(0,0,0,.7);border-radius:3px;padding:1px 4px;font-size:9px;font-weight:700}

/* MODAL */
.mbg{display:none;position:fixed;inset:0;background:rgba(0,0,0,.9);z-index:100;align-items:flex-end;justify-content:center}
.mbg.on{display:flex}
.mc{background:#111;border:1px solid #222;border-radius:22px 22px 0 0;width:100%;max-width:480px;padding:20px 18px calc(env(safe-area-inset-bottom,20px)+20px);animation:mup .3s cubic-bezier(.34,1.56,.64,1)}
@keyframes mup{from{transform:translateY(100%)}to{transform:none}}
.mc h2{font-size:17px;font-weight:700;margin-bottom:5px}
...
.mc ol b{color:#aaa}
.mc textarea{width:100%;background:var(--c2);border:1.5px solid var(--bd);border-radius:11px;color:#fff;font-family:monospace;font-size:11px;padding:11px;outline:none;resize:none;height:80px;margin-bottom:10px}
...
.mst{font-size:12px;padding:8px 12px;border-radius:8px;background:var(--c2);margin-bottom:10px;display:none}

/* TIP */
.tip{background:var(--c1);border:1px solid var(--bd);border-radius:13px;padding:11px 14px;display:flex;align-items:center;gap:10px}
.tip.gone{display:none}
.tip p{flex:1;font-size:12px;color:var(--sub);line-height:1.4}
.tip p b{color:#666}
.tipx{background:none;border:none;color:#2a2a2a;font-size:20px;cursor:pointer;padding:0;line-height:1}
</style>
</head>
<body>
...
<div id="app">
  ...
  <div class="scr">
    ...
    <div class="res" id="res">
      <!-- Preview -->
      <div class="pvwrap" id="pvwrap" onclick="togglePv()">
        <img id="pvthumb" src="" alt="">
        <video id="pvvid" preload="none" playsinline muted></video>
        <div class="pvplay" id="pvplay"><span>▶</span></div>
...
<div class="mbg" id="mbg" onclick="if(event.target===this)closeModal()">
  <div class="mc">
    <h2>🔑 Cookie Login</h2>
    <p>Für NSFW, Likes & Bookmarks brauchst du deine Twitter Cookies.</p>
    <ol>
    ...
    <div class="mrow">
      ...
      <button class="mb sec" id="delbtn" style="display:none" onclick="delCookies()">🗑</button>
      <button class="mb sec" id="chkbtn" style="display:none" onclick="chkCookies()">🔍</button>
...
window.addEventListener('load', () => {
  initStatus();
  setTimeout(() => { document.getElementById('splash').classList.add('out'); document.getElementById('app').classList.add('on'); }, 2600);
});
...
async function go(){
  ...
  if(tab==='profile'||tab==='likes'||tab==='saves') await loadProfile(url);
...
function renderMedia(d,tweetUrl){
  ...
  if(d.thumbnail){
    ...
    document.getElementById('pvbadge').textContent=d.type==='video'?'▶ Preview':'FOTO';
  ...
  if(d.formats&&d.formats.length){
    d.formats.forEach((f,i)=>{
      ...
      a.innerHTML='<div class="dll"><div class="di">↓</div><div><div class="dt">'+f.quality+'</div><div class="ds">MP4</div></div></div><span class="da">›</span>';
      c.appendChild(a);
    });
    ...
    c.innerHTML='<div style="text-align:center;color:#333;font-size:13px;padding:14px">Keine Medien. Cookies aktuell? 🔑</div>';
  }
...
function togglePv(){
  ...
  if(!_pvLoaded){
    ...
    const tick=()=>{ if(_pvTimer){ const p=Math.min((Date.now()-start)/dur*100,100); prog.style.width=p+'%'; if(p<100)_pvTimer=requestAnimationFrame(tick); } };
    _pvTimer=requestAnimationFrame(tick);
    setTimeout(()=>{
      vid.pause();
      play.classList.remove('hide');
      cancelAnimationFrame(_pvTimer);
    ...
    const tick=()=>{ if(_pvTimer){ const p=Math.min(cur+(Date.now()-start)/dur*100,100); prog.style.width=p+'%'; if(p<100)_pvTimer=requestAnimationFrame(tick); } };
    _pvTimer=requestAnimationFrame(tick);
    setTimeout(()=>{ vid.pause(); play.classList.remove('hide'); },dur);
  } else {
    vid.pause();
    play.classList.remove('hide');
    cancelAnimationFrame(_pvTimer);
...
async function loadProfile(url){
  setStat('Lade Profil… (15–30 Sek.)','loading');
  try{
...
function renderProfile(d){
  ...
  if(!d.items.length){
    c.innerHTML='<div style="text-align:center;color:#333;font-size:13px;padding:14px">Keine Medien. Cookies nötig?</div>';
  ...
  const g=document.createElement('div');g.className='pgrid2';
  d.items.forEach(item=>{
    const div=document.createElement('div');div.className='pitem';
    div.innerHTML='<img data-src="'+(item.thumbnail||'')+'" src="" alt=""><div class="pitov">↓</div>'+(item.type==='video'?'<div class="pvid">▶</div>':'');
...
async function chkCookies(){
  const el=document.getElementById('mst');el.style.display='block';el.textContent='Prüfe…';el.style.color='#555';
  try{const r=await api('/cookies/status');const d=await r.json();el.style.color=d.active?'#00e87a':'#ff3b3b';el.textContent=d.active?'✓ Cookies aktiv':'✗ Keine Cookies';}catch(e){el.style.color='#ff3b3b';el.textContent=e.message;}
}
...
function reset(){
  ...
  cancelAnimationFrame(_pvTimer);_pvUrl=null;_pvLoaded=false;
